...
- Buka tab `Experts`.
- Pastikan muncul log `ACANE LOCKED PROFILE v1.04 GUARDED`.
- Jika masih muncul `v1.02 PROFIT30` atau `AcaneM1_v1_profit30`, berarti VPS masih memakai file lama.

Catatan risiko: profile ini tetap agresif, tetapi build terbaru memakai daily loss guard `15%`, account circuit `15%`, open-risk cap `12%`, same-side cap `8%`, max same-side positions `4`, basket stop `9%`, dan cooldown setelah `2` fast-loss beruntun. Batas ini bukan garansi absolut karena slippage, gap, spread, koneksi, atau close failure bisa membuat loss melewati angka guard.
