...
#property strict
// Acane M1: AcaneM1_v1_quality_fast_4264: generated Acane tuning test.
...
const int    ACANE_MaxPositions = 3;
const int    ACANE_MaxSameSidePositions = 1;
const int    ACANE_MinSecondsBetweenEntries = 90;
...
const double ACANE_DailyMaxLossPct = 8.00;
...
const double ACANE_MaxAccountDrawdownPct = 10.00;
const bool   ACANE_CloseOnCircuitStop = true;
const double ACANE_MaxOpenRiskPct = 6.00;
const double ACANE_MaxSameSideOpenRiskPct = 4.00;
const double ACANE_BasketLossStopPct = 4.00;
const bool   ACANE_StopDayOnBasketLoss = true;
const int    ACANE_FastLossCooldownAfter = 1;
const int    ACANE_FastLossCooldownSeconds = 2700;
const double ACANE_MrvMaxImpulseATR = 0.65;
const double ACANE_MrvMaxEmaDistanceATR = 0.85;
...
const string ACANE_BlockEntryHours = "2,3,5,11,20";
...
const double ACANE_MeanReversionRSILow = 40.00;
const double ACANE_MeanReversionRSIHigh = 60.00;
const double ACANE_MeanReversionTouchATR = 0.10;
...
double AcaneLotForRisk(const double stopDistance, const double riskMultiplier)
  {
   ...
   double raw = riskCash / (stopDistance * AcaneValuePerPriceUnit());
   return(AcaneFloorVolume(raw));
  }
...
int OnInit()
  {
   ...
   AcaneResetWeeklyGuardIfNeeded();
   AcaneResetMonthlyGuardIfNeeded();
   PrintFormat("AcaneM1_v1_quality_fast_4264 PROFILE | symbol=%s tf=M1 risk=%.2f maxPos=%d sameSide=%d dailyLoss=%.2f accountCircuit=%.2f openRisk=%.2f basketStop=%.2f spreadMax=%.2f statusEvery=%d debugEvery=%d",
               _Symbol,
               ACANE_RiskPercent,
               ACANE_MaxPositions,
