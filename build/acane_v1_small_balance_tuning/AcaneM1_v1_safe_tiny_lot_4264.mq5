...
#property strict
// Acane M1: AcaneM1_v1_safe_tiny_lot_4264: generated Acane tuning test.

#include <Trade/Trade.mqh>

const double ACANE_RiskPercent = 5.00;
...
const double ACANE_DailyMaxLossPct = 5.00;
...
const double ACANE_MaxAccountDrawdownPct = 8.00;
const bool   ACANE_CloseOnCircuitStop = true;
const double ACANE_MaxOpenRiskPct = 3.00;
const double ACANE_MaxSameSideOpenRiskPct = 3.00;
const double ACANE_BasketLossStopPct = 3.00;
const bool   ACANE_StopDayOnBasketLoss = true;
const int    ACANE_FastLossCooldownAfter = 1;
const int    ACANE_FastLossCooldownSeconds = 3600;
...
const double ACANE_MeanReversionRiskMultiplier = 0.03;
...
int OnInit()
  {
   ...
   AcaneResetWeeklyGuardIfNeeded();
   AcaneResetMonthlyGuardIfNeeded();
   PrintFormat("AcaneM1_v1_safe_tiny_lot_4264 PROFILE | symbol=%s tf=M1 risk=%.2f maxPos=%d sameSide=%d dailyLoss=%.2f accountCircuit=%.2f openRisk=%.2f basketStop=%.2f spreadMax=%.2f statusEvery=%d debugEvery=%d",
               _Symbol,
               ACANE_RiskPercent,
               ACANE_MaxPositions,
