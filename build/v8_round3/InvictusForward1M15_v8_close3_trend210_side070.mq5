...
input double V7_TrendRiskPercent = 2.10;
input double V7_SidewaysRiskPercent = 0.70;
...
static const bool   V8_ENABLE_BEARISH_REGIME = true;
static const double V8_BEARISH_REGIME_ADX_MIN = 28.0;
static const int    V8_BEARISH_REGIME_CLOSE_LOOKBACK = 3;
static const int    V8_BEARISH_BUY_MIN_SCORE = 101;
static const bool   V8_BEARISH_BUY_REQUIRE_IMPULSIVE_BOS = true;
static const int    V8_BEARISH_SELL_MIN_SCORE = 80;
static const bool   V8_BEARISH_ALLOW_ALL_SELL_HOURS = true;
static const double V8_BEARISH_BUY_RISK_MULT = 0.00;
static const double V8_BEARISH_SELL_RISK_MULT = 1.25;
static const bool   V8_DISABLE_SIDEWAYS_IN_BEARISH_REGIME = true;
...
bool IF1IsBearishRegimeH4()
  {
   if(!V8_ENABLE_BEARISH_REGIME)
      return(false);

   MqlRates h4Rates[];
   ArraySetAsSeries(h4Rates, true);
   if(CopyRates(_Symbol, PERIOD_H4, 0, 24, h4Rates) < 12)
      return(false);

   IF1BiasInfo h4Bias = IF1DetectBias(h4Rates);
   if(h4Bias.direction != IF1_BIAS_BEARISH)
      return(false);

   double adxH4 = IF1GetAdxH4();
   if(adxH4 < V8_BEARISH_REGIME_ADX_MIN)
      return(false);

   int lookback = MathMin(V8_BEARISH_REGIME_CLOSE_LOOKBACK, ArraySize(h4Rates) - 1);
   if(lookback < 2)
      return(false);

   if(h4Rates[1].close >= h4Rates[lookback].close)
      return(false);
   if(h4Rates[1].high >= h4Rates[lookback].high)
      return(false);
...
bool IF1EvaluateTrending()
  {
   ...
   bool bearishRegime = IF1IsBearishRegimeH4();
   if(bias.direction == IF1_BIAS_BEARISH &&
      !V7_ALLOW_ALL_SELL_HOURS &&
      !(bearishRegime && V8_BEARISH_ALLOW_ALL_SELL_HOURS) &&
   ...
   int buyMinScore = bearishRegime ? V8_BEARISH_BUY_MIN_SCORE : IF1_SCORE_BUY_MIN_OVERRIDE;
   int sellMinScore = bearishRegime ? V8_BEARISH_SELL_MIN_SCORE : IF1_SCORE_SELL_MIN_OVERRIDE;

   if(bias.direction > 0 && score < buyMinScore)
     {
      g_diag.trendScoreBlocked++;
      return(false);
     }
   if(bias.direction < 0 && (score < sellMinScore || bos.quality != IF1_BOS_IMPULSIVE))
     {
      g_diag.trendScoreBlocked++;
      return(false);
     }
   if(bias.direction > 0 && bearishRegime && V8_BEARISH_BUY_REQUIRE_IMPULSIVE_BOS && bos.quality != IF1_BOS_IMPULSIVE)
     {
      g_diag.trendScoreBlocked++;
      return(false);
     }
   ...
   if(bearishRegime)
     {
      if(sell)
         riskUsd *= V8_BEARISH_SELL_RISK_MULT;
      else
         riskUsd *= V8_BEARISH_BUY_RISK_MULT;
     }
...
bool IF1EvaluateSideways()
  {
   g_diag.sidewaysChecks++;
   if(V8_DISABLE_SIDEWAYS_IN_BEARISH_REGIME && IF1IsBearishRegimeH4())
     {
      g_diag.sidewaysAdxBlocked++;
      return(false);
     }
