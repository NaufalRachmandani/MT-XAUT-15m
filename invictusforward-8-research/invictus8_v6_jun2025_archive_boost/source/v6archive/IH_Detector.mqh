                                        ...
                                        int totalBars)
{
   ...
   if(refBar + 10 >= totalBars) return IH_BIAS_NEUTRAL;
   ...
   for(int i = swingBar - 1; i >= refBar + 1; i--)
   {
      ...
      if(broken)
      {
         ...
         if(info.quality != IH_BOS_NONE)
         {
            ...
            //--- Sweep (find opposite swing for liquidity grab)
            double sweepTarget = 0;
            int    sweepBar = 0;
            if(lookForBullish)
               FindSwingLow(i, IH_SwingLookback, 20, low, totalBars, sweepTarget, sweepBar);
            else
               FindSwingHigh(i, IH_SwingLookback, 20, high, totalBars, sweepTarget, sweepBar);
            info.hasSweep = (sweepTarget > 0) ?
               DetectSweep(i, sweepTarget, lookForBullish, high, low, close, totalBars) : false;
