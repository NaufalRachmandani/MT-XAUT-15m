//+------------------------------------------------------------------+
//|                     InvictusForward-8-V6ArchiveBoost.mq5       |
...
int OnInit()
{
   ...
   LogOpen();
   LogWrite("[INIT] InvictusForward-8-V6ArchiveBoost v6.00 M15 — Jun2025 archive guard experiment");
...
void CheckDropCatcher()
{
   ...
   gdc_trade.Sell(lot, _Symbol, 0, dcSL, dcTP, "IF8V6A_DC DropCatch");
...
void ProcessTrending(datetime barTime, MqlDateTime &dt, bool isSideways)
{
   ...
   double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   string comment = StringFormat("IF8V6A_TR S:%d %s", score,
...
void ProcessSideways(datetime barTime, MqlDateTime &dt)
{
   ...
   string comment = StringFormat("IF8V6A_RB %s R:$%.0f", dir, rangeSize);
