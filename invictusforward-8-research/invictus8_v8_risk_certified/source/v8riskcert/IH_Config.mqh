...
input double Trend_RRStandard        = 1.5;    // RR for standard Trend setup
input double Trend_RRStrong          = 2.0;    // RR for score >= 90
input double Trend_RRTop             = 2.5;    // RR for score >= 100
...
//=== V5 Quality Boost ===
input group "══════ V5 Quality Boost ══════"
input bool   V5_EnableTrendQualityScaling = true; // Scale lot by score/hour leak profile
input double V5_TrendScore85LotMult       = 0.65; // Score <=85 trend lot multiplier
input double V5_TrendScore90LotMult       = 0.85; // Score <=90 trend lot multiplier
input double V5_TrendScore95LotMult       = 1.00; // Score <=95 trend lot multiplier
input double V5_TrendHour09LotMult        = 0.75; // H09 trend leak dampener
input double V5_TrendHour12LotMult        = 0.80; // H12 trend leak dampener
input double V5_TrendHour15LotMult        = 0.60; // H15 trend leak dampener
input bool   V5_BlockScore85Hour09        = false; // Optional hard block for weak H09 trend
input bool   V5_BlockScore90Hour12        = false; // Optional hard block for weak H12 trend
input bool   V5_BlockScore85Hour15        = false; // Optional hard block for weak H15 trend

//=== V6 Jun2025 Archive Guard ===
input group "══════ V6 Archive Guard ══════"
input bool   V6_EnableArchiveHourScaling  = false; // Enable Jun2025-now archive hour risk controls
input double V6_TrendHour07LotMult        = 1.00;  // Archive: H07 trend multiplier
input double V6_TrendHour08LotMult        = 1.00;  // Archive: H08 trend multiplier
input double V6_TrendHour13LotMult        = 1.00;  // Archive: H13 trend multiplier
input double V6_TrendHour16LotMult        = 1.00;  // Archive: H16 trend multiplier
input bool   V6_BlockTrendHour13          = false; // Archive: hard block H13 trend cluster
input bool   V6_BlockTrendHour16          = false; // Archive: hard block H16 trend cluster
input bool   V6_BlockRangeBounceHour01    = false; // Archive: block RB H01 loss cluster
input bool   V6_BlockRangeBounceHour17    = false; // Archive: block RB H17 loss cluster

//=== V8 Risk Guard ===
input group "══════ V8 Risk Guard (V7 input names preserved) ══════"
input bool   V7_EnableRiskGuard           = false; // Enable equity/risk throttle for live candidate tests
input double V7_EquityDDSoftPct           = 12.0;  // Peak-equity DD where lot throttle starts
input double V7_EquityDDSoftLotMult       = 0.50;  // Lot multiplier after soft DD threshold
input double V7_EquityDDHardPct           = 25.0;  // Peak-equity DD where stronger throttle starts
input double V7_EquityDDHardLotMult       = 0.25;  // Lot multiplier after hard DD threshold
input double V7_StopTradingDDPct          = 55.0;  // Peak-equity DD where new entries stop
input double V7_DailyEquityLossPct        = 6.0;   // Day-start equity loss where entries pause for day
input double V7_LargeLossPct              = 5.0;   // Closed-loss percent that triggers cooldown
input int    V7_LargeLossCooldownBars     = 16;    // Global cooldown after large loss
input int    V7_MaxTotalPositions         = 4;     // Max open positions across modules, 0 disables
input double V7_BalanceBelowLotCap        = 20000.0; // Apply low-balance lot cap below this balance
input double V7_LowBalanceMaxLot          = 1.25;  // Max lot while balance below V7_BalanceBelowLotCap
input bool   V7_EnablePatternGuard        = false; // Enable Jun-Dec 2025 pattern leak filters
input bool   V7_BlockTrendHour08          = false; // Block Trend H08 leak cluster
input bool   V7_BlockTrendScore85Hour07   = false; // Block low-score Trend H07 leak cluster
input bool   V7_BlockTrendScore85Hour15   = false; // Block low-score Trend H15 leak cluster
input bool   V7_BlockTrendScore95Hour06   = false; // Block top-score Trend H06 loss cluster
input bool   V7_BlockTrendScore95Hour08   = false; // Block top-score Trend H08 loss cluster
input bool   V7_BlockTrendScore95Hour12   = false; // Block top-score Trend H12 loss cluster
input bool   V7_BlockTrendScore95Hour16   = false; // Block top-score Trend H16 loss cluster
input bool   V7_BlockRangeBounceWeakHours = false; // Block weak RB hours 00/02/07/16/22
input bool   V7_BlockDropCatcherWeakHours = false; // Block weak DC hours 02/05/06/09/11/13/15
input bool   V7_EnablePatternScaling      = false; // Scale leak clusters instead of hard-blocking them
input double V7_TrendHour08LotMult        = 1.00;  // Soft scale all Trend H08 entries
input double V7_TrendScore85Hour07LotMult = 1.00;  // Soft scale Trend score<=85 H07
input double V7_TrendScore85Hour15LotMult = 1.00;  // Soft scale Trend score<=85 H15
input double V7_TrendScore95Hour06LotMult = 1.00;  // Soft scale Trend score>=95 H06
input double V7_TrendScore95Hour08LotMult = 1.00;  // Soft scale Trend score>=95 H08
input double V7_TrendScore95Hour12LotMult = 1.00;  // Soft scale Trend score>=95 H12
input double V7_TrendScore95Hour16LotMult = 1.00;  // Soft scale Trend score>=95 H16
input double V7_RangeBounceWeakHourLotMult = 1.00; // Soft scale RB weak hours
input double V7_DropCatcherWeakHourLotMult = 1.00; // Soft scale DC weak hours
