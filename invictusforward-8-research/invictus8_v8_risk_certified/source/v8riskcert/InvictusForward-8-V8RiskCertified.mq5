...
//  V7 LIVE RISK STATE
//============================================================
double         g_v7PeakEquity       = 0.0;
double         g_v7DayStartEquity   = 0.0;
datetime       g_v7CooldownUntil    = 0;
...
bool V7WeakDropCatcherHour(int hour)
{
   if(!V7_EnablePatternGuard || !V7_BlockDropCatcherWeakHours) return false;
   return (hour == 2 || hour == 5 || hour == 6 || hour == 9 || hour == 11 || hour == 13 || hour == 15);
}

bool V7WeakRangeBounceHour(int hour)
{
   if(!V7_EnablePatternGuard || !V7_BlockRangeBounceWeakHours) return false;
   return (hour == 0 || hour == 2 || hour == 7 || hour == 16 || hour == 22);
}

bool V7IsWeakDropCatcherHour(int hour)
{
   return (hour == 2 || hour == 5 || hour == 6 || hour == 9 || hour == 11 || hour == 13 || hour == 15);
}

bool V7IsWeakRangeBounceHour(int hour)
{
   return (hour == 0 || hour == 2 || hour == 7 || hour == 16 || hour == 22);
}

double V7ClampPatternMult(double mult)
{
...
void V7UpdateRiskState(bool newDay)
{
   if(!V7_EnableRiskGuard) return;

   double equity = AccountInfoDouble(ACCOUNT_EQUITY);
   if(equity <= 0.0) return;

   if(g_v7PeakEquity <= 0.0 || equity > g_v7PeakEquity)
      g_v7PeakEquity = equity;

   if(newDay || g_v7DayStartEquity <= 0.0)
      g_v7DayStartEquity = equity;
}

double V7PeakEquityDDPct()
{
   if(!V7_EnableRiskGuard || g_v7PeakEquity <= 0.0) return 0.0;

   double equity = AccountInfoDouble(ACCOUNT_EQUITY);
   if(equity <= 0.0 || equity >= g_v7PeakEquity) return 0.0;
   return (g_v7PeakEquity - equity) / g_v7PeakEquity * 100.0;
}

double V7DailyEquityLossPct()
{
   if(!V7_EnableRiskGuard || g_v7DayStartEquity <= 0.0) return 0.0;

   double equity = AccountInfoDouble(ACCOUNT_EQUITY);
   if(equity <= 0.0 || equity >= g_v7DayStartEquity) return 0.0;
   return (g_v7DayStartEquity - equity) / g_v7DayStartEquity * 100.0;
}

int V7OpenRiskPositions()
{
   int count = 0;
   for(int i = 0; i < PositionsTotal(); i++)
   {
      ...
      if(magic == EA_MAGIC_BT || magic == EA_MAGIC_SIDEWAYS || magic == EA_MAGIC_DC)
         count++;
   }
   return count;
}

bool V7RiskBlocksEntries(string module)
{
   if(!V7_EnableRiskGuard) return false;

   datetime now = TimeCurrent();
   if(g_v7CooldownUntil > 0 && now < g_v7CooldownUntil)
   {
      LogWrite("[V8-SKIP] " + module + " global cooldown until " + TimeToString(g_v7CooldownUntil, TIME_DATE|TIME_MINUTES));
      return true;
   }

   double peakDd = V7PeakEquityDDPct();
   if(V7_StopTradingDDPct > 0.0 && peakDd >= V7_StopTradingDDPct)
   {
      LogWrite("[V8-SKIP] " + module + " peak equity DD " + DoubleToString(peakDd,2) + "% >= " + DoubleToString(V7_StopTradingDDPct,2) + "%");
      return true;
   }

   double dailyLoss = V7DailyEquityLossPct();
   if(V7_DailyEquityLossPct > 0.0 && dailyLoss >= V7_DailyEquityLossPct)
   {
      LogWrite("[V8-SKIP] " + module + " daily equity loss " + DoubleToString(dailyLoss,2) + "% >= " + DoubleToString(V7_DailyEquityLossPct,2) + "%");
      return true;
   }

   if(V7_MaxTotalPositions > 0 && V7OpenRiskPositions() >= V7_MaxTotalPositions)
   {
      LogWrite("[V8-SKIP] " + module + " max total positions " + IntegerToString(V7OpenRiskPositions()) + "/" + IntegerToString(V7_MaxTotalPositions));
      return true;
   }
...
double V7RiskLotMultiplier()
{
   if(!V7_EnableRiskGuard) return 1.0;

   double peakDd = V7PeakEquityDDPct();
   if(V7_EquityDDHardPct > 0.0 && peakDd >= V7_EquityDDHardPct)
      return MathMax(0.01, MathMin(V7_EquityDDHardLotMult, 1.0));
   if(V7_EquityDDSoftPct > 0.0 && peakDd >= V7_EquityDDSoftPct)
      return MathMax(0.01, MathMin(V7_EquityDDSoftLotMult, 1.0));
   return 1.0;
}

double V7ApplyLotGuard(double lot)
{
   if(!V7_EnableRiskGuard) return NormalizeDouble(lot, 2);

   double balance = AccountInfoDouble(ACCOUNT_BALANCE);
   if(V7_BalanceBelowLotCap > 0.0 && balance < V7_BalanceBelowLotCap && V7_LowBalanceMaxLot > 0.0)
      lot = MathMin(lot, V7_LowBalanceMaxLot);

   lot *= V7RiskLotMultiplier();
   if(lot < 0.01) lot = 0.01;
   return NormalizeDouble(lot, 2);
}

void V7RegisterClosedLoss(double lossAbs)
{
   if(!V7_EnableRiskGuard) return;
   if(lossAbs <= 0.0 || V7_LargeLossPct <= 0.0 || V7_LargeLossCooldownBars <= 0) return;

   double balanceBeforeLoss = AccountInfoDouble(ACCOUNT_BALANCE) + lossAbs;
   if(balanceBeforeLoss <= 0.0) return;

   double lossPct = lossAbs / balanceBeforeLoss * 100.0;
   if(lossPct >= V7_LargeLossPct)
   {
      g_v7CooldownUntil = TimeCurrent() + V7_LargeLossCooldownBars * PeriodSeconds(_Period);
      LogWrite("[V8-COOLDOWN] loss " + DoubleToString(lossPct,2) + "% -> " + TimeToString(g_v7CooldownUntil, TIME_DATE|TIME_MINUTES));
   }
}
...
double V5TrendQualityLotMultiplier(int score, int hour)
{
   ...
   if(V6_EnableArchiveHourScaling)
   {
      if(V6_BlockTrendHour13 && hour == 13)
         return 0.0;
      if(V6_BlockTrendHour16 && hour == 16)
         return 0.0;

      if(hour == 7)
         mult *= V6_TrendHour07LotMult;
      else if(hour == 8)
         mult *= V6_TrendHour08LotMult;
      else if(hour == 13)
         mult *= V6_TrendHour13LotMult;
      else if(hour == 16)
         mult *= V6_TrendHour16LotMult;
   }

   if(V7_EnablePatternGuard)
   {
      if(V7_BlockTrendHour08 && hour == 8)
         return 0.0;
      if(V7_BlockTrendScore85Hour07 && hour == 7 && score <= 85)
         return 0.0;
      if(V7_BlockTrendScore85Hour15 && hour == 15 && score <= 85)
         return 0.0;
      if(V7_BlockTrendScore95Hour06 && hour == 6 && score >= 95)
         return 0.0;
      if(V7_BlockTrendScore95Hour08 && hour == 8 && score >= 95)
         return 0.0;
      if(V7_BlockTrendScore95Hour12 && hour == 12 && score >= 95)
         return 0.0;
      if(V7_BlockTrendScore95Hour16 && hour == 16 && score >= 95)
         return 0.0;

      if(V7_EnablePatternScaling)
      {
         if(hour == 8)
            mult *= V7ClampPatternMult(V7_TrendHour08LotMult);
         if(hour == 7 && score <= 85)
            mult *= V7ClampPatternMult(V7_TrendScore85Hour07LotMult);
         if(hour == 15 && score <= 85)
            mult *= V7ClampPatternMult(V7_TrendScore85Hour15LotMult);
         if(hour == 6 && score >= 95)
            mult *= V7ClampPatternMult(V7_TrendScore95Hour06LotMult);
         if(hour == 8 && score >= 95)
            mult *= V7ClampPatternMult(V7_TrendScore95Hour08LotMult);
         if(hour == 12 && score >= 95)
            mult *= V7ClampPatternMult(V7_TrendScore95Hour12LotMult);
         if(hour == 16 && score >= 95)
            mult *= V7ClampPatternMult(V7_TrendScore95Hour16LotMult);
      }
   }
...
int OnInit()
{
   ...
   g_v7PeakEquity = AccountInfoDouble(ACCOUNT_EQUITY);
   g_v7DayStartEquity = g_v7PeakEquity;
   g_v7CooldownUntil = 0;

   LogOpen();
   LogWrite("[INIT] InvictusForward-8-V8RiskCertified v8.00 M15 - risk-certified candidate");
...
void CheckDropCatcher()
{
   ...
   if(V7WeakDropCatcherHour(dcDt.hour)) return;
   if(V7RiskBlocksEntries("DC")) return;
   ...
   if(V7_EnablePatternGuard && V7_EnablePatternScaling && V7IsWeakDropCatcherHour(dcDt.hour))
      lot = NormalizeDouble(lot * V7ClampPatternMult(V7_DropCatcherWeakHourLotMult), 2);
   if(lot < 0.01) lot = 0.01;
   if(lot > BT_MaxLotCap) lot = BT_MaxLotCap;
   lot = V7ApplyLotGuard(lot);

   gdc_trade.Sell(lot, _Symbol, 0, dcSL, dcTP, "IF8V8R_DC DropCatch");
...
void ProcessTrending(datetime barTime, MqlDateTime &dt, bool isSideways)
{
   ...
   if(V7RiskBlocksEntries("TR")) return;
   ...
   double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   string comment = StringFormat("IF8V8R_TR S:%d %s", score,
...
void ProcessSideways(datetime barTime, MqlDateTime &dt)
{
   ...
   if(V7RiskBlocksEntries("RB")) return;
   ...
   if(V6_EnableArchiveHourScaling)
   {
      if(dt.hour == 1 && V6_BlockRangeBounceHour01) return;
      if(dt.hour == 17 && V6_BlockRangeBounceHour17) return;
   }
   if(V7WeakRangeBounceHour(dt.hour)) return;
   ...
   if(V7_EnablePatternGuard && V7_EnablePatternScaling && V7IsWeakRangeBounceHour(dt.hour))
      lot = NormalizeDouble(lot * V7ClampPatternMult(V7_RangeBounceWeakHourLotMult), 2);
   if(lot < 0.01) lot = 0.01;
   if(lot > BT_MaxLotCap) lot = BT_MaxLotCap;
   lot = V7ApplyLotGuard(lot);
   ...
   string comment = StringFormat("IF8V8R_RB %s R:$%.0f", dir, rangeSize);
         ...
         if(dealEntry == DEAL_ENTRY_OUT)
         {
            ...
            if(magic == EA_MAGIC_BT)
            {
               ...
               else
               {
                  ...
                  g_dailyLossCount++;
                  V7RegisterClosedLoss(MathAbs(profit));
...
double OnTester()
{
   ...
   int handle = FileOpen(
      "InvictusForward8_V8RiskCertified_optimization_passes.csv",
   ...
   if(handle != INVALID_HANDLE)
   {
      if(FileSize(handle) <= 2)
      {
         FileWrite(handle,
                   ...
                   "V6_EnableArchiveHourScaling", "V6_TrendHour07LotMult", "V6_TrendHour08LotMult",
                   "V6_TrendHour13LotMult", "V6_TrendHour16LotMult",
                   "V6_BlockTrendHour13", "V6_BlockTrendHour16",
                   "V6_BlockRangeBounceHour01", "V6_BlockRangeBounceHour17",
                   "V7_EnablePatternGuard", "V7_EnablePatternScaling", "V7_TrendHour08LotMult",
                   "V7_TrendScore85Hour07LotMult", "V7_TrendScore85Hour15LotMult",
                   "V7_TrendScore95Hour06LotMult", "V7_TrendScore95Hour08LotMult",
                   "V7_TrendScore95Hour12LotMult", "V7_TrendScore95Hour16LotMult",
                   "V7_RangeBounceWeakHourLotMult", "V7_DropCatcherWeakHourLotMult");
      }
