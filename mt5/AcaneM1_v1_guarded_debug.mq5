...
#property strict
// Acane M1: aggressive tick-managed XAUUSD scalper with diagnostic logging.
...
int OnInit()
  {
   ...
   AcaneResetWeeklyGuardIfNeeded();
   AcaneResetMonthlyGuardIfNeeded();
   PrintFormat("ACANE LOCKED PROFILE v1.04 GUARDED DEBUG | symbol=%s tf=M1 risk=%.2f maxPos=%d sameSide=%d dailyLoss=%.2f accountCircuit=%.2f openRisk=%.2f basketStop=%.2f spreadMax=%.2f statusEvery=%d debugEvery=%d",
               _Symbol,
               ACANE_RiskPercent,
               ACANE_MaxPositions,
