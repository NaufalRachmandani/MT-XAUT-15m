...
#property strict
// Acane M1 guarded debug throttle: aggressive MRV scalper with stricter stacking and loss controls.
...
const double ACANE_DailyMaxLossPct = 6.00;
...
const double ACANE_MaxOpenRiskPct = 4.00;
...
int OnInit()
  {
   ...
   AcaneResetWeeklyGuardIfNeeded();
   AcaneResetMonthlyGuardIfNeeded();
   PrintFormat("ACANE PROFILE v1.05 GUARDED DEBUG THROTTLE | symbol=%s tf=M1 risk=%.2f maxPos=%d sameSide=%d dailyLoss=%.2f accountCircuit=%.2f openRisk=%.2f basketStop=%.2f spreadMax=%.2f statusEvery=%d debugEvery=%d",
               _Symbol,
               ACANE_RiskPercent,
               ACANE_MaxPositions,
