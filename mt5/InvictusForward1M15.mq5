...
static const int    IF1_TIMED_PROFIT_CLOSE_HOURS_OVERRIDE = 3;
static const double IF1_SIDEWAYS_TP_FRACTION_OVERRIDE = 0.65;
static const double IF1_SIDEWAYS_MIN_TP_USD = 8.0;
static const double IF1_REFERENCE_GOLD_CONTRACT_SIZE = 100.0;
static const double IF1_TREND_MIN_SL_OVERRIDE = 15.0;
static const double IF1_TREND_MAX_SL_OVERRIDE = 35.0;
...
bool IF1InCooldown(const IF1StrategyState &state)
  {
   if(state.lastLossTime <= 0)
      return(false);
   return((TimeCurrent() - state.lastLossTime) < (IF1_COOLDOWN_BARS * PeriodSeconds(PERIOD_M15)));
  }
...
bool IF1EvaluateTrending()
  {
   ...
   if(g_trendState.dayStartBalance > 0.0 && g_trendState.dailyLossUsd > (g_trendState.dayStartBalance * IF1_TREND_DAILY_LOSS_CAP_PCT / 100.0))
     {
      g_diag.trendDailyLossBlocked++;
      return(false);
     }
   ...
   if(bias.direction == IF1_BIAS_BEARISH && !IF1HourInArray(IF1HourOf(TimeCurrent()), IF1TrendSellHours))
     {
      g_diag.trendBearishHourBlocked++;
      return(false);
     }
   ...
   if(bias.direction > 0 && score < IF1_SCORE_BUY_MIN)
     {
      g_diag.trendScoreBlocked++;
      return(false);
     }
   if(bias.direction < 0 && (score < IF1_SCORE_SELL_MIN || bos.quality != IF1_BOS_IMPULSIVE))
     {
      g_diag.trendScoreBlocked++;
      return(false);
     }
...
bool IF1EvaluateSideways()
  {
   g_diag.sidewaysChecks++;
   if(IF1GetAdxH4() >= IF1_SIDEWAYS_ADX_THRESHOLD)
     {
      g_diag.sidewaysAdxBlocked++;
      return(false);
     }
   ...
   if(g_sidewaysState.dayStartBalance > 0.0 && g_sidewaysState.dailyLossUsd > (g_sidewaysState.dayStartBalance * IF1_SIDEWAYS_DAILY_LOSS_CAP_PCT / 100.0))
     {
      g_diag.sidewaysDailyLossBlocked++;
      return(false);
     }
   ...
   if(IF1CountOpenPositions(IF1_SIDEWAYS_MAGIC) >= IF1_SIDEWAYS_MAX_POSITIONS)
     {
      g_diag.sidewaysOpenPositionsBlocked++;
      return(false);
     }
   ...
   double buyZoneTop = rangeLow + (rangeSize * IF1_SIDEWAYS_ENTRY_ZONE_FRACTION);
   ...
   if(IF1SidewaysClusterCount(rangeHigh, rangeLow) >= IF1_SIDEWAYS_CLUSTER_MAX_PER_DAY)
     {
      g_diag.sidewaysClusterBlocked++;
      return(false);
     }
