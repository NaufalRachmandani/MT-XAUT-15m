...
static const int    IF1_TIMED_PROFIT_CLOSE_HOURS_OVERRIDE = 2;
static const double IF1_SIDEWAYS_TP_FRACTION_OVERRIDE = 0.60;
static const double IF1_SIDEWAYS_MIN_TP_USD = 7.0;
...
static const int    IF1_SCORE_RR_BOOST_MIN_OVERRIDE = 88;
static const double IF1_SELL_LOT_MODIFIER_OVERRIDE = 0.85;
static const double IF1_SIDEWAYS_LOT_MODIFIER_OVERRIDE = 0.15;
...
static const int    IF1_TREND_KILLSWITCH_WINS_OVERRIDE = 40;
...
double IF1BaseCompoundingLot(const double balance)
  {
   double lot = 0.01;
   if(balance <= 20000.0)
      lot = MathFloor(balance / 275.0) * 0.01;
   else if(balance <= 50000.0)
      lot = 0.72 + MathFloor((balance - 20000.0) / 350.0) * 0.01;
   else
      lot = 1.58 + MathFloor((balance - 50000.0) / 500.0) * 0.01;
...
bool IF1EvaluateTrending()
  {
   ...
   if(IF1HourInArray(IF1HourOf(TimeCurrent()), IF1TrendToxicHours))
     {
      g_diag.trendToxicHourBlocked++;
      return(false);
     }
   ...
   if(bias.direction == IF1_BIAS_BEARISH && !IF1_ALLOW_ALL_SELL_HOURS &&
      !IF1HourInArray(IF1HourOf(TimeCurrent()), IF1TrendSellHours))
     {
      g_diag.trendBearishHourBlocked++;
      return(false);
     }
   ...
   double rr = (score >= IF1_SCORE_RR_BOOST_MIN_OVERRIDE) ? IF1_TREND_TP_RR_BOOST : IF1_TREND_TP_RR_DEFAULT;
...
bool IF1EvaluateSideways()
  {
   ...
   if(IF1HourInArray(IF1HourOf(TimeCurrent()), IF1SidewaysToxicHours))
     {
      g_diag.sidewaysToxicHourBlocked++;
      return(false);
     }
