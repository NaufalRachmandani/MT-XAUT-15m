...
static const int    IF1_SCORE_SELL_MIN_OVERRIDE = 75;
static const int    IF1_TREND_LOW_SCORE_SELL_MAX = 84;
static const double IF1_TREND_LOW_SCORE_SELL_RR_OVERRIDE = 1.35;
static const double IF1_TREND_LOW_SCORE_SELL_LOT_MULT = 0.50;
...
static const double IF1_SELL_LOT_MODIFIER_OVERRIDE = 0.75;
static const double IF1_SIDEWAYS_LOT_MODIFIER_OVERRIDE = 0.10;
static const double IF1_TREND_DAILY_LOSS_CAP_PCT_OVERRIDE = 8.0;
static const double IF1_SIDEWAYS_DAILY_LOSS_CAP_PCT_OVERRIDE = 5.0;
...
static const double IF1_BUY_MIN_BREAK_ATR_OVERRIDE = 0.24;
...
int IF1MaxTrendingPositions(const double balance)
  {
   if(balance <= 20000.0)
      return(IF1_TREND_MAX_POSITIONS_LOW_OVERRIDE);
   if(balance <= 50000.0)
      return(IF1_TREND_MAX_POSITIONS_MID_OVERRIDE);
   return(IF1_TREND_MAX_POSITIONS_HIGH_OVERRIDE);
  }
...
double IF1StrategyLot(const bool sideways, const bool sell, const IF1StrategyState &state)
  {
   ...
   if(sideways)
      lot *= IF1_SIDEWAYS_LOT_MODIFIER_OVERRIDE;
   if(sell)
      lot *= IF1_SELL_LOT_MODIFIER_OVERRIDE;
   ...
   if(!sell)
     {
      ...
      signal.stopLoss = IF1NormalizePrice(ask - signal.stopDistance);
      signal.takeProfit = IF1NormalizePrice(ask + (signal.stopDistance * signal.rr));
      ok = g_trade.Buy(signal.lot, _Symbol, 0.0, signal.stopLoss, signal.takeProfit, comment);
     }
   else
     {
      signal.useLimitOrder = false;
      signal.stopLoss = IF1NormalizePrice(bid + signal.stopDistance);
      signal.takeProfit = IF1NormalizePrice(bid - (signal.stopDistance * signal.rr));
...
bool IF1EvaluateTrending()
  {
   ...
   if(g_trendState.dayStartBalance > 0.0 && g_trendState.dailyLossUsd > (g_trendState.dayStartBalance * IF1_TREND_DAILY_LOSS_CAP_PCT_OVERRIDE / 100.0))
     {
      g_diag.trendDailyLossBlocked++;
      return(false);
     }
   ...
   double lot = IF1StrategyLot(false, bias.direction < 0, g_trendState);
   if(bias.direction < 0 && score <= IF1_TREND_LOW_SCORE_SELL_MAX)
      lot = IF1NormalizeVolume(lot * IF1_TREND_LOW_SCORE_SELL_LOT_MULT);
   if(bias.direction > 0 && score <= IF1_BUY_LOW_SCORE_MAX)
      lot = IF1NormalizeVolume(lot * IF1_BUY_LOW_SCORE_LOT_MULT);
   if(bias.direction < 0 && score >= IF1_TREND_POSITIVE_SELL_MIN && score <= IF1_TREND_POSITIVE_SELL_MAX)
      lot = IF1NormalizeVolume(lot * IF1_TREND_POSITIVE_SELL_LOT_MULT);
   if(bias.direction > 0 && score >= IF1_TREND_HIGH_SCORE_LOT_BOOST_MIN)
      lot = IF1NormalizeVolume(lot * IF1_TREND_HIGH_SCORE_LOT_BOOST);
   if(bias.direction < 0 && score >= IF1_TREND_HIGH_SCORE_LOT_BOOST_MIN)
      lot = IF1NormalizeVolume(lot * IF1_TREND_HIGH_SCORE_SELL_LOT_MULT);
   if(lot < IF1_MIN_LOT)
     {
      g_diag.trendLotBlocked++;
      return(false);
   ...
   if(signal.direction > 0)
     {
      signal.stopLoss = IF1NormalizePrice(zone.entryPrice - stopDistance);
      signal.takeProfit = IF1NormalizePrice(zone.entryPrice + (stopDistance * rr));
     }
   else
     {
      signal.stopLoss = IF1NormalizePrice(zone.entryPrice + stopDistance);
...
bool IF1EvaluateSideways()
  {
   ...
   if(g_sidewaysState.dayStartBalance > 0.0 && g_sidewaysState.dailyLossUsd > (g_sidewaysState.dayStartBalance * IF1_SIDEWAYS_DAILY_LOSS_CAP_PCT_OVERRIDE / 100.0))
     {
      g_diag.sidewaysDailyLossBlocked++;
      return(false);
     }
   ...
   if(IF1CountOpenPositions(IF1_SIDEWAYS_MAGIC) >= IF1_SIDEWAYS_MAX_POSITIONS_OVERRIDE)
     {
      g_diag.sidewaysOpenPositionsBlocked++;
      return(false);
     }
   ...
   double lot = IF1StrategyLot(true, false, g_sidewaysState);
   if(lot < IF1_MIN_LOT)
     {
      g_diag.sidewaysLotBlocked++;
      return(false);
