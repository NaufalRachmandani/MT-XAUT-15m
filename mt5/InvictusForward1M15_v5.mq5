...
bool IF1IsVeryLowBalance(const double balance)
  {
   return(balance <= IF1_VERY_LOW_BALANCE_CUTOFF_OVERRIDE);
  }


int IF1MaxTrendingPositions(const double balance)
  {
   if(IF1IsVeryLowBalance(balance))
      return(IF1_TREND_MAX_POSITIONS_VERY_LOW_OVERRIDE);
   if(balance <= 20000.0)
...
int IF1CurrentSidewaysMaxPositions(const double balance)
  {
   if(IF1IsVeryLowBalance(balance))
      return(IF1_SIDEWAYS_MAX_POSITIONS_VERY_LOW_OVERRIDE);
   return(IF1_SIDEWAYS_MAX_POSITIONS_OVERRIDE);
  }
...
double IF1CurrentTrendDailyLossCapPct(const double balance)
  {
   if(IF1IsVeryLowBalance(balance))
      return(IF1_TREND_DAILY_LOSS_CAP_PCT_VERY_LOW_OVERRIDE);
   return(IF1_TREND_DAILY_LOSS_CAP_PCT_OVERRIDE);
  }
...
double IF1CurrentSidewaysDailyLossCapPct(const double balance)
  {
   if(IF1IsVeryLowBalance(balance))
      return(IF1_SIDEWAYS_DAILY_LOSS_CAP_PCT_VERY_LOW_OVERRIDE);
   return(IF1_SIDEWAYS_DAILY_LOSS_CAP_PCT_OVERRIDE);
  }
