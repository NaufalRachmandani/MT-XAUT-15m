...
EA_SOURCE = ROOT / "mt5" / "InvictusForward1M15_v3.mq5"
TOOLS_DIR = ROOT / "tools"
RUN_DIR = ROOT / "build" / "v3_20x"
...
PARAMS: dict[str, Any] = {
    ...
    "IF1_SIDEWAYS_TP_FRACTION_OVERRIDE": 0.60,
    "IF1_SIDEWAYS_MIN_TP_USD": 7.0,
    ...
    "IF1_SCORE_RR_BOOST_MIN_OVERRIDE": 86,
    "IF1_SELL_LOT_MODIFIER_OVERRIDE": 0.95,
    "IF1_SIDEWAYS_LOT_MODIFIER_OVERRIDE": 0.10,
    "IF1_TREND_DAILY_LOSS_CAP_PCT_OVERRIDE": 8.5,
    ...
    "IF1_TREND_TP_RR_DEFAULT_OVERRIDE": 1.60,
    "IF1_TREND_TP_RR_BOOST_OVERRIDE": 2.20,
    "IF1_TREND_HIGH_SCORE_LOT_BOOST_MIN": 95,
    "IF1_TREND_HIGH_SCORE_LOT_BOOST": 1.14,
}
...
ITERATIONS: list[dict[str, Any]] = [
    {
        "name": "baseline",
        "hypothesis": "Baseline v3 final sebagai champion awal.",
        "changes": {},
    },
    {
        "name": "trend_hold_5h",
        "hypothesis": "Biarkan winner trend hidup lebih lama.",
        "changes": {
            "IF1_TREND_TIMED_PROFIT_CLOSE_HOURS_OVERRIDE": 5,
            "IF1_TREND_TIMED_PROFIT_USD_PER_001_OVERRIDE": 26.0,
        },
    },
    {
        "name": "trend_rr_plus",
        "hypothesis": "Naikkan RR trend sedikit tanpa mengubah gate.",
        "changes": {
    ...
    {
        "name": "high_score_boost_up",
        "hypothesis": "High-confidence trend bisa dibebani lebih berat.",
        "changes": {
            "IF1_TREND_HIGH_SCORE_LOT_BOOST": 1.18,
        },
    },
    {
        "name": "trend_capacity_up",
        "hypothesis": "Posisi trend masih bisa ditambah tanpa membuka gate baru.",
        "changes": {
            "IF1_TREND_MAX_POSITIONS_LOW_OVERRIDE": 8,
            "IF1_TREND_MAX_POSITIONS_MID_OVERRIDE": 12,
            "IF1_TREND_MAX_POSITIONS_HIGH_OVERRIDE": 16,
        },
    },
    {
        "name": "high_balance_compound_up",
        "hypothesis": "Growth akhir periode bisa didorong lewat compounding high-balance.",
        "changes": {
            "IF1_COMPOUND_BALANCE_STEP_HIGH_OVERRIDE": 340.0,
            "IF1_COMPOUND_BASE_LOT_HIGH_OVERRIDE": 1.98,
        },
    },
    {
        "name": "sideways_lot_down",
        "hypothesis": "Sideways tetap dipakai, tapi bobotnya dikecilkan lagi.",
        "changes": {
            "IF1_SIDEWAYS_LOT_MODIFIER_OVERRIDE": 0.08,
        },
    },
    {
        "name": "sell_fuller",
        "hypothesis": "Sell masih bisa dinaikkan sedikit untuk seimbangkan sisi bearish.",
        "changes": {
            "IF1_SELL_LOT_MODIFIER_OVERRIDE": 1.00,
        },
    },
    {
        "name": "high_score_and_sell",
        "hypothesis": "Kombinasi sell lebih penuh dan boost trend score tinggi.",
        "changes": {
            "IF1_SELL_LOT_MODIFIER_OVERRIDE": 1.00,
            "IF1_TREND_HIGH_SCORE_LOT_BOOST": 1.18,
        },
    ...
    {
        "name": "capacity_and_sideways_down",
        "hypothesis": "Tambah kapasitas trend tapi imbangi dengan sideways lebih ringan.",
        "changes": {
            "IF1_TREND_MAX_POSITIONS_LOW_OVERRIDE": 8,
            "IF1_TREND_MAX_POSITIONS_MID_OVERRIDE": 12,
            "IF1_TREND_MAX_POSITIONS_HIGH_OVERRIDE": 16,
    ...
    {
        "name": "rr_boost_trigger_lower",
        "hypothesis": "Lebih banyak trend layak dapat RR boost.",
        "changes": {
            "IF1_SCORE_RR_BOOST_MIN_OVERRIDE": 84,
        },
    },
    {
        "name": "mid_compound_up",
        "hypothesis": "Dorong compounding pada balance menengah.",
        "changes": {
            "IF1_COMPOUND_BALANCE_STEP_MID_OVERRIDE": 275.0,
            "IF1_COMPOUND_BASE_LOT_MID_OVERRIDE": 0.86,
        },
    },
    {
        "name": "trend_daily_cap_up",
        "hypothesis": "Beri ruang sedikit lebih luas untuk growth trend.",
        "changes": {
            "IF1_TREND_DAILY_LOSS_CAP_PCT_OVERRIDE": 9.0,
        },
    },
    {
        "name": "trend_sl_tighter",
        "hypothesis": "SL sedikit lebih rapat bisa menaikkan expectancy bila noise masih tertahan.",
        "changes": {
            "IF1_TREND_MIN_SL_OVERRIDE": 11.0,
            "IF1_TREND_MAX_SL_OVERRIDE": 28.0,
        },
    },
    {
        "name": "sideways_tp_trim",
        "hypothesis": "Kurangi keserakahan sideways agar lebih cepat realize profit.",
        "changes": {
    ...
    {
        "name": "trend_hold_6h",
        "hypothesis": "Uji apakah trend winner masih bisa diekstrak lebih lama.",
        "changes": {
            "IF1_TREND_TIMED_PROFIT_CLOSE_HOURS_OVERRIDE": 6,
            "IF1_TREND_TIMED_PROFIT_USD_PER_001_OVERRIDE": 28.0,
        },
    },
    {
        "name": "trend_hold_5h_soft",
        "hypothesis": "Versi lebih lunak dari hold yang lebih panjang.",
        "changes": {
            "IF1_TREND_TIMED_PROFIT_CLOSE_HOURS_OVERRIDE": 5,
            "IF1_TREND_TIMED_PROFIT_USD_PER_001_OVERRIDE": 24.0,
        },
    ...
    {
        "name": "capacity_hold_combo",
        "hypothesis": "Tambah ruang posisi dan hold trend serempak.",
        "changes": {
            "IF1_TREND_MAX_POSITIONS_LOW_OVERRIDE": 8,
            "IF1_TREND_MAX_POSITIONS_MID_OVERRIDE": 12,
            "IF1_TREND_MAX_POSITIONS_HIGH_OVERRIDE": 16,
    ...
    {
        "name": "trend_growth_combo",
        "hypothesis": "Paket growth: sell penuh, high-score boost lebih besar, sideways lebih tipis.",
        "changes": {
            "IF1_SELL_LOT_MODIFIER_OVERRIDE": 1.00,
            "IF1_TREND_HIGH_SCORE_LOT_BOOST": 1.18,
            "IF1_SIDEWAYS_LOT_MODIFIER_OVERRIDE": 0.08,
    ...
    },
    {
        ...
        "hypothesis": "Kombinasi final untuk menantang champion tanpa membuka gate terlalu liar.",
        "changes": {
            "IF1_TREND_MAX_POSITIONS_LOW_OVERRIDE": 8,
            "IF1_TREND_MAX_POSITIONS_MID_OVERRIDE": 12,
            "IF1_TREND_MAX_POSITIONS_HIGH_OVERRIDE": 16,
...
def acceptable(metrics: dict[str, float]) -> bool:
    return metrics["profit_factor"] >= 1.10 and metrics["equity_dd_pct"] <= 58.0 and metrics["total_trades"] >= 3000


def composite(metrics: dict[str, float]) -> float:
    dd_penalty = 1.0 + max(0.0, metrics["equity_dd_pct"] - 45.0) / 20.0
...
def better(metrics: dict[str, float], best: dict[str, float]) -> bool:
    ...
    if not acceptable(best):
        return True
    if metrics["net_profit"] > best["net_profit"] * 1.015 and metrics["profit_factor"] >= best["profit_factor"] - 0.03 and metrics["equity_dd_pct"] <= best["equity_dd_pct"] + 8.0:
        return True
    if metrics["net_profit"] > best["net_profit"] * 0.99 and metrics["profit_factor"] > best["profit_factor"] + 0.02 and metrics["equity_dd_pct"] <= best["equity_dd_pct"] - 4.0:
        return True
    return composite(metrics) > composite(best) * 1.03


def summarize_takeaway(metrics: dict[str, float], best: dict[str, float], trend: dict[str, float], sideways: dict[str, float]) -> str:
    if not acceptable(metrics):
        if metrics["equity_dd_pct"] > 58.0:
    ...
    if metrics["net_profit"] > best["net_profit"] and metrics["equity_dd_pct"] <= best["equity_dd_pct"] + 5.0:
    ...
    if trend.get("dailyLoss", 0.0) > 1500:
...
def write_summary(results: list[IterationResult], champion: IterationResult) -> None:
    json_path = RUN_DIR / "v3_20x_results.json"
    md_path = RUN_DIR / "v3_20x_summary.md"
    ...
    lines = [
        "# v3 20x Iteration Summary",
        "",
        f"Champion: iterasi {champion.index:02d} `{champion.name}`",
        "",
        "| Iter | Name | Accepted | Net Profit | PF | Trades | Eq DD | Takeaway |",
        "| --- | --- | --- | ---: | ---: | ---: | ---: | --- |",
    ]
    for r in results:
        lines.append(
            ...
            f"{int(r.metrics['total_trades'])} | {r.metrics['equity_dd_pct']:.2f}% | {r.takeaway} |"
        )
    md_path.write_text("\n".join(lines) + "\n")
...
def main() -> None:
    ...
    for idx, plan in enumerate(ITERATIONS, start=1):
        ...
        stem = f"InvictusForward1M15_v3_iter{idx:02d}"
        ...
        if champion_metrics is None:
            ...
            takeaway = summarize_takeaway(metrics, champion_metrics, trend_diag, sideways_diag)
