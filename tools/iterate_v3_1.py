...
EA_SOURCE = ROOT / "mt5" / "InvictusForward1M15_v3_1.mq5"
TOOLS_DIR = ROOT / "tools"
RUN_DIR = ROOT / "build" / "v3_1_5x"
...
ITERATIONS: list[dict[str, Any]] = [
    {
        "name": "baseline",
        "hypothesis": "Baseline champion v3 sebagai pembanding awal v3.1.",
        "changes": {},
    },
    {
        "name": "sideways_tp_trim",
        "hypothesis": "Ambil profit sideways lebih cepat untuk menurunkan drawdown tanpa mengganggu layer trend.",
        "changes": {
    ...
    {
        "name": "sideways_trim_plus_lot_down",
        "hypothesis": "Kombinasi trim TP sideways dan lot sideways lebih kecil untuk memangkas DD lebih jauh.",
        "changes": {
            "IF1_SIDEWAYS_TP_FRACTION_OVERRIDE": 0.55,
            "IF1_SIDEWAYS_MIN_TP_USD": 6.5,
    ...
    {
        "name": "sell_tame",
        "hypothesis": "Kurangi eksposur sell karena pass lama menunjukkan sell penuh merusak expectancy.",
        "changes": {
            "IF1_SELL_LOT_MODIFIER_OVERRIDE": 0.90,
        },
    },
    {
        "name": "trend_daily_cap_down",
        "hypothesis": "Turunkan daily loss cap trend sedikit untuk memotong ekor drawdown.",
        "changes": {
            "IF1_TREND_DAILY_LOSS_CAP_PCT_OVERRIDE": 8.0,
        },
    },
    {
        "name": "defensive_combo",
        "hypothesis": "Paket defensif lengkap: trim sideways, sell lebih kecil, dan daily cap trend lebih ketat.",
        "changes": {
            "IF1_SIDEWAYS_TP_FRACTION_OVERRIDE": 0.55,
            "IF1_SIDEWAYS_MIN_TP_USD": 6.5,
            "IF1_SIDEWAYS_LOT_MODIFIER_OVERRIDE": 0.08,
...
def acceptable(metrics: dict[str, float]) -> bool:
    return metrics["profit_factor"] >= 1.13 and metrics["equity_dd_pct"] <= 50.0 and metrics["total_trades"] >= 3400


def hits_target(metrics: dict[str, float]) -> bool:
    return (
        metrics["net_profit"] >= 450000.0
        and metrics["equity_dd_pct"] <= 35.0
        and metrics["profit_factor"] >= 1.14
        and metrics["total_trades"] >= 3400
    )


def composite(metrics: dict[str, float]) -> float:
    dd_penalty = 1.0 + max(0.0, metrics["equity_dd_pct"] - 35.0) / 8.0
...
def better(metrics: dict[str, float], best: dict[str, float]) -> bool:
    ...
    if not acceptable(best):
        return True
    cur_target = hits_target(metrics)
    best_target = hits_target(best)
    ...
    if best_target and not cur_target:
        return False
    if metrics["net_profit"] >= 450000.0 and metrics["equity_dd_pct"] <= best["equity_dd_pct"] - 2.0 and metrics["profit_factor"] >= best["profit_factor"] - 0.02:
        return True
...
def summarize_takeaway(metrics: dict[str, float], best: dict[str, float], trend: dict[str, float], sideways: dict[str, float]) -> str:
    if not acceptable(metrics):
        if metrics["equity_dd_pct"] > 50.0:
            return "Ditolak: drawdown melewati guardrail."
        if metrics["profit_factor"] < 1.13:
            return "Ditolak: profit factor terlalu turun."
        if metrics["total_trades"] < 3400:
            return "Ditolak: trade count turun terlalu jauh."
        return "Ditolak: gagal guardrail utama."
    if hits_target(metrics):
        return "Target tercapai: profit tetap tinggi dan DD masuk target."
...
def write_summary(results: list[IterationResult], champion: IterationResult) -> None:
    json_path = RUN_DIR / "v3_1_5x_results.json"
    md_path = RUN_DIR / "v3_1_5x_summary.md"
    ...
    lines = [
        "# v3.1 5 Experiment Summary",
        "",
        f"Champion: iterasi {champion.index:02d} `{champion.name}`",
        f"Target: net profit >= 450000, equity DD <= 35%, total trades >= 3400",
        "",
...
def main() -> None:
    ...
    for idx, plan in enumerate(ITERATIONS, start=1):
        ...
        stem = f"InvictusForward1M15_v3_1_iter{idx:02d}"
