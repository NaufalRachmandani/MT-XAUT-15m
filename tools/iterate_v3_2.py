...
EA_SOURCE = ROOT / "mt5" / "InvictusForward1M15_v3_2.mq5"
TOOLS_DIR = ROOT / "tools"
RUN_DIR = ROOT / "build" / "v3_2_5x"
...
PARAMS: dict[str, Any] = {
    ...
    "IF1_TREND_HIGH_SCORE_BUY_RR_OVERRIDE": 2.25,
...
ITERATIONS: list[dict[str, Any]] = [
    {
        "name": "baseline",
        "hypothesis": "Baseline v3.1 final sebagai pembanding awal v3.2.",
        "changes": {},
    },
    {
        "name": "sell_lighter_075",
        "hypothesis": "Sell masih negatif, jadi kita kecilkan eksposurnya lagi tanpa mematikan sisi bearish sepenuhnya.",
        "changes": {
    ...
    {
        "name": "sell_score_85",
        "hypothesis": "Entry sell yang terlalu marginal dibuang dengan menaikkan ambang score sell.",
        "changes": {
            "IF1_SCORE_SELL_MIN_OVERRIDE": 85,
        },
    },
    {
        "name": "high_buy_rr_245",
        "hypothesis": "High-score buy adalah winner terbaik, jadi TP-nya dipanjangkan sedikit lagi.",
        "changes": {
            "IF1_TREND_HIGH_SCORE_BUY_RR_OVERRIDE": 2.45,
        },
    },
    {
        "name": "sell_filter_plus_high_buy_rr",
        "hypothesis": "Gabungkan filter sell yang lebih ketat dengan TP lebih panjang hanya untuk buy premium.",
        "changes": {
            "IF1_SCORE_SELL_MIN_OVERRIDE": 85,
            "IF1_SELL_LOT_MODIFIER_OVERRIDE": 0.75,
            "IF1_TREND_HIGH_SCORE_BUY_RR_OVERRIDE": 2.45,
...
def acceptable(metrics: dict[str, float]) -> bool:
    return metrics["profit_factor"] >= 1.14 and metrics["equity_dd_pct"] <= 35.0 and metrics["total_trades"] >= 3200


def hits_target(metrics: dict[str, float]) -> bool:
    return (
        metrics["net_profit"] >= 470000.0
...
def better(metrics: dict[str, float], best: dict[str, float]) -> bool:
    ...
    if best_target and not cur_target:
        return False
    if metrics["net_profit"] >= 460000.0 and metrics["equity_dd_pct"] <= best["equity_dd_pct"] - 2.0 and metrics["profit_factor"] >= best["profit_factor"] - 0.01:
        return True
...
def write_summary(results: list[IterationResult], champion: IterationResult) -> None:
    json_path = RUN_DIR / "v3_2_5x_results.json"
    md_path = RUN_DIR / "v3_2_5x_summary.md"
    ...
    lines = [
        "# v3.2 5 Experiment Summary",
        "",
        f"Champion: iterasi {champion.index:02d} `{champion.name}`",
        f"Target: net profit >= 470000, equity DD <= 25%, total trades >= 3200",
        "",
...
def main() -> None:
    ...
    for idx, plan in enumerate(ITERATIONS, start=1):
        ...
        stem = f"InvictusForward1M15_v3_2_iter{idx:02d}"
