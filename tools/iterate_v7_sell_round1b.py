...
RUN_DIR = ROOT / "build" / "v7_sell_round1"
...
PRESETS = [
    Preset("s1", "control", "Current v7 champion control.", dict(BASE)),
    Preset("s2", "sell_mult_050", "Lower sell risk multiplier.", {**BASE, "V7_SellRiskMultiplier": "0.50"}),
    Preset("s3", "sell_mult_085", "Raise sell risk multiplier modestly.", {**BASE, "V7_SellRiskMultiplier": "0.85"}),
    Preset("s4", "sell_score_80", "Looser sell score gate.", {**BASE, "IF1_SCORE_SELL_MIN_OVERRIDE": "80"}),
    Preset("s5", "all_sell_hours", "Allow all sell hours.", {**BASE, "V7_ALLOW_ALL_SELL_HOURS": "true"}),
    Preset("s6", "all_sell_score80_mult085", "All sell hours plus looser score and bigger sell risk.", {
        **BASE,
        "V7_ALLOW_ALL_SELL_HOURS": "true",
        "IF1_SCORE_SELL_MIN_OVERRIDE": "80",
        "V7_SellRiskMultiplier": "0.85",
    }),
    Preset("s7", "sell_off", "Effectively disable sell sizing.", {**BASE, "V7_SellRiskMultiplier": "0.00"}),
]
...
def build_source(preset: Preset) -> Path:
    ...
    path = RUN_DIR / f"InvictusForward1M15_v7_{preset.code}.mq5"
...
def main() -> None:
    ...
    for preset in PRESETS:
        ...
        stem = compile_variant("v7", source_path, preset.code)
        print(f"[compile] {preset.code} {preset.name} -> {stem}", flush=True)
        ...
        print(
            f"[result] {result.code} {result.preset}: net={result.net_profit:.2f} pf={result.profit_factor:.2f} "
    ...
    lines = [
        ...
        "",
        "| Rank | Code | Preset | Net | Profit % | PF | WR | Trades | EqDD |",
        "| ---: | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for idx, item in enumerate(ranked, start=1):
        lines.append(
            f"| {idx} | {item.code} | {item.preset} | {item.net_profit:.2f} | {item.profit_pct:.2f}% | "
